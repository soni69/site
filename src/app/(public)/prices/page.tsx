...
import { matchDeviceCategory } from '@/components/sections/device-categories'
...
export const metadata: Metadata = {
  title: `Цены на ремонт — ${BRAND_CONFIG.companyName}`,
  description:
...
function PriceCategoryTable({ group }: { group: PriceCategory }) {
  const device = matchDeviceCategory(group.category)
  const DeviceIcon = device?.Icon
  // Якорь для перехода с главной страницы (Hero device icons)
  const anchorId = device ? `device-${device.key}` : `category-${group.id}`

  return (
    <section
      aria-labelledby={`category-${group.id}`}
      id={anchorId}
      className="overflow-hidden rounded-xl border border-border scroll-mt-24"
    >
      {/* Заголовок категории */}
      <div className="flex items-center gap-3 border-b border-border bg-muted/50 px-4 py-3 sm:px-6">
        {DeviceIcon && (
          <span
